...
func Test_HttpMatcher_StoringARequest(t *testing.T) {
	...
	ident := "c->s:123->456:42"
	reqTimestamp := time.Now()
	req := &http.Request{}

	entry, found := matcher.GetOrStoreRequest(ident, reqTimestamp, req)
...
func Test_HttpMatcher_StoringAResponse(t *testing.T) {
	...
	entry, found := matcher.GetOrStoreResponse(ident, resTimestamp, res)
...
func Test_HttpMatcher_GetResponseThatMatchesRequest(t *testing.T) {
	matcher := newRequestResponseMatcher()
	tcpIdent := "c->s:123->456:42"

	// store a response that won't match
	_, found := matcher.GetOrStoreResponse("I am matchless", time.Now(), &http.Response{})
	assert.False(t, found, "Expect no matching request when storing a matchless response.")

	// store the response that will match
	resp := &http.Response{}
	_, found = matcher.GetOrStoreResponse(tcpIdent, time.Now(), resp)
	...
	req := &http.Request{}
	foundEntry, found := matcher.GetOrStoreRequest(tcpIdent, time.Now(), req)
...
func Test_HttpMatcher_GetRequestThatMatchesResponse(t *testing.T) {
	...
	_, found := matcher.GetOrStoreRequest("I am matchless", time.Now(), &http.Request{})
	assert.False(t, found, "Expect no matching response when storing a matchless request.")

	// store the request that will match
	req := &http.Request{}
	_, found = matcher.GetOrStoreRequest(tcpIdent, time.Now(), req)
	...
	resp := &http.Response{}
	foundEntry, found := matcher.GetOrStoreResponse(tcpIdent, time.Now(), resp)
